...
libtge.simplify_graph.argtypes = [ctypes.POINTER(ctypes.c_char), ctypes.POINTER(ctypes.c_uint32), ctypes.POINTER(ctypes.c_char), ctypes.c_uint32]
...
def simplify_graph(graph_def, sinks=["GradientDescent"]):
    ...
    sinks_raw = ' '.join(sinks).encode('ascii')
    libtge.simplify_graph(graph_raw_mut, graph_len_mut, sinks_raw, len(sinks_raw))
